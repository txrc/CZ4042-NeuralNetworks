...
def train(beta):
    ...
    with tf.Session() as sess:
        ...
        for i in range(epochs):
            ...
            if i % 100 == 0:
                print('The total training errors for epoch {} is {} with beta parameter of {}'.format(i, train_error[i], beta)) # Testing acc
    return train_error
        
def main():
    beta = [0, 10e-3, 10e-6, 10e-9, 10e-12]

    no_threads = mp.cpu_count()
    p = mp.Pool(processes = no_threads)
    paras = p.map(train, beta)
    
    paras = np.array(paras)

    plt.figure()
    for i in range(len(beta)):
        plt.plot(range(epochs), paras[i], label='= {}'.format(beta[i]))

    plt.title("Training Errors vs. No. Of Epochs for all decay parameters")
    plt.xlabel('Epochs')
